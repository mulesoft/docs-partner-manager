...
To use a map, create the map in Anypoint Studio and then import the map to the xref:maps.adoc[Maps] page in Partner Manager.

To start with an example of a map, download and then customize the https://mulesoft.com/exchange/com.mulesoft.muleesb.modules/b2b-order-to-cash-mapping/minor/1.0/[B2B/EDI: Order to Cash - Mapping Examples^] from Exchange. This example includes the following transformations for message flows:
...
[[change-map]]
== Select the Map for a Partner Manager Message Flow

When you configure a message flow, you can select the `.dwl` file when adding the map from the Maps page or on the *Select a map* page of the message flow. You can also xref:maps.adoc#version-a-map[version the map] and change the map selection on a deployed message flow.
