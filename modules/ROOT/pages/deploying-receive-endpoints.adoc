...
== Deploy to CloudHub or Runtime Fabric on a Public Port
...
== Deploy to CloudHub or Runtime Fabric on a Private Port
...
* Set CloudHub or Runtime Fabric to automatically deploy newly created endpoints to a private port, as described in xref:cloudhub-deployment-settings.adoc[] or xref:rtf-deployment-setting.adoc[].
