...
* To keep a custom public ingress URL stable across redeployments for AS2 or HTTP inbound endpoints, enable *Preserve Public URL of Ingress application* in the endpoint settings.
+
If this option isn't enabled, Partner Manager can regenerate the public ingress URL when redeploying the endpoint or message flow.
...
Partner Manager deploys AS2 and HTTP inbound endpoints to Runtime Fabric with an HTTP listener. You can expose HTTPS endpoints for internal and external users by using Runtime Fabric network configurations and Ingress rules.
