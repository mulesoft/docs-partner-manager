...
| HTTPS
a| Indicates whether the AS2 endpoint receives data via HTTPS or HTTP:

* If enabled (default), the AS2 endpoint receives data via HTTPS.
* If disabled, the AS2 endpoint receives data via HTTP.
