...
|Enable this setting to manage the endpoint and its access control with API Manager, which allows you to apply policies to the endpoint, such as authentication and client credentials, and configure SLA tiers. Document your message specifications by using Exchange and share them with your partners via public Exchange portals or by obtaining an additional license and sharing the specifications in Anypoint API Experience Hub.

To manage an endpoint with API Manager, create an HTTP API in API Manager in the same environment and organization as the endpoint.
...
** If you follow the xref:api-experience-hub::requesting-access-to-an-api-instance.adoc[API request process] on behalf of your partners, HTTP basic auth headers pass along the client ID and client secret upon request.
