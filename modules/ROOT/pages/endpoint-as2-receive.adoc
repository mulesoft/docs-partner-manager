...
|[[manage-api-manager]]Manage with API Manager
...
|App Name Substitution Type
a|Defines how the ingress URL template `app-name` is substituted for this AS2 endpoint to generate the Runtime Fabric public endpoint.

* Select Default to use `app-name` as deployment-name.
* Select Custom to provide a custom name for the `app-name` substitution.
* Select BasePath to use the editable AS2 base path as the value for the `app-name` substitution.
| No
