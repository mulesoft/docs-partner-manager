...
|Private port when deploying to Anypoint Virtual Private Cloud (VPC).
a|
* When enabled, the AS2 receive endpoint application deploys to the private port `8092` in Mule in your xref:runtime-manager::virtual-private-cloud.adoc[VPC]. You can manage the access control to the endpoint via the VPC and dedicated load balancer settings.
...
This configuration is applicable only to VPC deployments from Partner Manager.
|No
...
If your organization uses VPC without a dedicated load balancer (DLB), your organization’s platform administrator can configure the necessary xref:runtime-manager::vpc-firewall-rules-concept.adoc[VPC firewall rules] to allow specific partner or 3rd party connection IP addresses to access the endpoint.
