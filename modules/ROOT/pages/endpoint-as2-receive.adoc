...
|Private port (when deploying to Anypoint VPC)
a|
...
When deploying to Anypoint Runtime Fabric, Partner Manager always deploys the AS2 receive endpoint as an HTTP listener. With a Runtime Fabric deployment, you must configure an Ingress controller to provide external consumers with an HTTPS endpoint. For more information, see xref::runtime-fabric:custom-ingress-configuration.adoc[Configuring Ingress for Runtime Fabric].
| x
