...
Your organization's AS2 receive endpoint settings determine where on the host to receive inbound data that comes either directly from partners or from 3rd party connections. One AS2 endpoint can receive inbound data from multiple partners, 3rd party connections, or both. Only a host can own AS2 receive endpoints.

Create AS2 receive endpoints by configuring these settings on the host or Partners page:
