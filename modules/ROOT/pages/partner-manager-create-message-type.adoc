...
* `hostReferenceId` +
Optional, except when backend systems are sending outbound transactions on behalf of different internal business units that require different mapping or sender identifiers to be set on the EDI data. This field is not used for the receive from partners message type.
* `businessDocumentKey` +
