...
You can create up to five custom message attributes for Receive from Partners and Source at Host message types. This functionality enables you to track additional values in the inbound payload. To enable this functionality, you must add definitions for these attributes in the DataWeave map for the message flow.
