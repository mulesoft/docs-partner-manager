...
|Report non-fatal errors in acknowledgements
|When checked, the AK3 segment of the generated 997 functional acknowledgment reports non-fatal errors.

When unchecked, the AK3 segment of the generated 997 functional acknowledgment doesn't report non-fatal errors.
...
|If selected and *Fail when value length outside allowed range* isn't selected, Partner Manager truncates the data elements that exceed the maximum length limit in the generated output.
|Disabled
...
|If selected, Partner Manager includes elements from the Interchange Trailer (IEA) and Group Trailer (GE) segments of the X12 data payload.
|Disabled

|Ignore included segments
|If selected, the X12 read operation output includes segments with usage type 'Ignore', and you can use the segment s in message flow maps.
|Disabled
