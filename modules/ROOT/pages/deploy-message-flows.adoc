...
After Partner Manager validates the message flow configuration elements, deploy and test the message flow in a sandbox environment. The changes can take up to five minutes from the time the message flow deployment completes in the UI to take effect in the runtime.
