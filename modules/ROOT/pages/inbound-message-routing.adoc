...
The process that Partner Manager uses to route inbound messages differs depending on whether the messages are EDI messages (Edifact or X12) or non-EDI messages (CSV, JSON, or XML). 

=== EDI Messages

Partner Manager routes an inbound EDI message (EDIFACT or X12) based on the EDI identifier (ISA identifier for X12; UNB identifier for EDIFACT) in the message content.

=== Non-EDI Messages

Partner Manager routes an inbound non-EDI message (CSV, JSON, or XML) based on either the associated message type or the filename:
...
When Partner Manager deploys endpoint applications in  Mule runtime receive inbound messages from your partners, the applications go through a series of steps based on the protocol and endpoint configurations. Then, the document processing service routes the message via the `dps-inbound-queue`. The API replication service receives a copy of the message via the `tracking-queue`, stores the message payload in your storage, and updates the tracking information in Partner Manager.

diagram placeholder

=== Document Processing

The document processing application uses the inbound message's attributes, such as content-type header, URL relative path, file name, and payload, to route the message, as the diagram shows. When Partner Manager doesn't find a matching message flow, the transmission receives a *Failed* status and an error message shows.
