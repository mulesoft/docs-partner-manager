...
* If you did not specify an associated message type in the endpoint configuration, Partner Manager routes the message based on the filename. The filename should begin with `{message-type-id}_` have an appropriate file extension:
+
** `.json` for JSON files
** `.xml` for XML files
