...
Partner Manager uses this criteria to route inbound messages:
...
To specify the message type for incoming AS2 messages, your partner:

* Uses a filename that begins with `{message-type-id}_` for non-EDI files (JSON or XML).
+
...
To specify the message type for incoming HTTP or HTTPS messages, your partner:

* Posts EDI messages (EDIFACT or X12) to the base URL and sets the Content-Type value in the incoming HTTP request as follows, depending on the message format:
...
+
For example, if you have this scenario in a CloudHub deployment:
...
[NOTE]
When creating a Reference Map for an endpoint, the Partner and Host Reference IDs are automatically accessible within the DataWeave script. You can use these IDs to dynamically assign the sender and receiver identifiers or to apply conditional logic based on the specific partner configuration.
