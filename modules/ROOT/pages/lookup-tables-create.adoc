...
Up to 200 lookup tables can be created per environment. Two types of lookup tables are supported:

* *Composite Key Lookup*
+
A composite key lookup table is used to search for a single row in the table by using a combination of columns. This type uses AND logic, so both Value A and Value B must be provided to get a result.
+
* *Cross-Reference Lookup*
+
A cross-reference lookup table is used to search for multiple rows in the table by using a combination of columns. Choose this type when you have multiple unique identifiers for the same thing and need to find the record using any one of them. This type uses OR logic, so Value A, B, or C can be provided to get the result.
...
After creating a lookup table, you can add up to 10,000 rows. You can also import data from a CSV file and export or delete the table.

To add a row, click *Add Row* and enter up to 200 characters for the value in each column. New rows are added to the top of the table.
...
. Choose to append the data to the existing table or replace the existing table. If you replace the table, the existing data is deleted and can't be recovered.
. Click *Import*.

== Export a Lookup Table

To export a lookup table, click *Export CSV*. The table is exported immediately and can be downloaded as a CSV file.
