...
When deploying to Anypoint Runtime Fabric, Partner Manager always deploys this endpoint as an HTTP listener. With a Runtime Fabric deployment, you must configure an Ingress controller to provide external consumers with an HTTPS endpoint. For more information, see xref::runtime-fabric:custom-ingress-configuration.adoc[Configuring Ingress for Runtime Fabric].
|x
