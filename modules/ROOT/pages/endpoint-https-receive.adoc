...
+
You can manage the access control to the endpoint by using the Anypoint VPC and dedicated load balancer settings.
...
a| Enable this setting to manage the endpoint with API Manager. To use this functionality, you must must do the following:

* Create an HTTP API in API Manager in the same environment as the endpoint.
* Enter an instance ID for the API in the *API Auto discovery id* field.

Note: Using this feature requires API Manager entitlement.
