...
|Preserve Public URL of Ingress application
a|Applies to CloudHub 2.0 inbound endpoint deployments.

When enabled, Partner Manager preserves the user-defined public URL for the ingress application across endpoint or message flow redeployments.

When disabled, Partner Manager can regenerate the public ingress URL during redeployment.
|No
...
[NOTE]
====
* When deploying to Anypoint Runtime Fabric, Partner Manager deploys the HTTP or HTTPS receive endpoint as an HTTP listener.
** You must configure an Ingress controller to provide external consumers with an HTTPS endpoint. For more information, see xref:runtime-fabric::custom-ingress-configuration.adoc[Configuring Ingress for Runtime Fabric].

* When deploying to CloudHub 2.0, Partner Manager deploys the HTTP or HTTPS Receive endpoint as an HTTPS listener. 
** When using a shared space as a deployment target, HTTP requests aren't allowed.
** To keep a user-defined public ingress URL stable across redeployments, select *Preserve Public URL of Ingress application* in the endpoint settings.
** If you want to allow HTTP requests to the HTTP or HTTPS receive endpoint application:
...
include::partial$https-receive.adoc[]
