...
HTTP and HTTPS receive endpoint settings configure where to receive messages where to receive or send the following types of data:
...
You can also set CloudHub to automatically deploy newly created HTTP and HTTPS endpoints to a private port via the xref:cloudhub-deploy-options.adoc[CloudHub Deployment Settings].
