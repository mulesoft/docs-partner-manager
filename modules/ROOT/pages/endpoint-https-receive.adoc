...
You can view this URL by selecting the receiving endpoint from your deployed message flow or by clicking the endpoint name from the endpoints list. This URL can be shared with the partners that would send HTTP or HTTPS messages to this endpoint.
...
If your backend processes consuming the HTTP or HTTPS receive endpoint are also running inside the same VPC, they can call the internal URL directly.
