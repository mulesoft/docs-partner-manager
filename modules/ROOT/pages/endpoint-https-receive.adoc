...
If your backend processes consuming the endpoint are in a different network, your organization’s platform administrator can update DLB settings to xref:runtime-manager::lb-whitelists.adoc[whitelist] specific IP addresses to access the endpoint.
...
image::URL-mapping-rules-http.png[URL mapping rules in the DLB]
