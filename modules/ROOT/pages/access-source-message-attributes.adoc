...
+
Accessing transmission IDs in the translation maps enables end-to-end tracing because these IDs are passed to the translated JSON or XML output file.
+
...
+
For example, you can access the source filename or a specific HTTP header value as a custom message attribute in in the message type's reference map to easily search for these values in the Activity screens.
