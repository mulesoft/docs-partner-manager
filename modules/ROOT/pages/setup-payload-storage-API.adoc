...
NOTE: You must make the payload storage API publicly accessible if you want the payload to be available from the Partner Manager Activity page. If your payload storage API can only be accessed in your network, then you must use a different implementation of the API. See <<payload-view, Payload view URL>> for more information.
