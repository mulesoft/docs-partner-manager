...
_**Note to Maria** - Please add an image of the Anypoint Platform page that shows Partner Manager on the _
...
* You need to designate a set of rumtimes to deploy the B2B Gateway i.e. the Mule app that processes your B2B transactions. If your Mule runtimes are not on CloudHub, setup a server group via Anypoint Runtime Manager. The steps to setup your server group are in the Runtime doc: https://docs.mulesoft.com/runtime-manager/server-group-create.

NOTE: The server group associated to your B2B Gateway need to be named "B2BServerGroup" (name is case sensitive).

* A storage for your B2B message payloads. Anypoint Partner Manager enables you to connect your own storage (such as a database, Amazon S3, Azure Blob Storage, and so forth). To connect to your storage, you will need to establish an API that can be registered with Anypoint Partner Manager to connect to the storage.
