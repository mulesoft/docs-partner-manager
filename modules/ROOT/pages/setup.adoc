...
* Storage API solutions that are accessible from Mule runtime.
...
There is one Administrator role name Partner Manager Administrator. Depending on which environment you select, the user is assigned that role within the context of that environment.
...
. Download the Partner Manager Content Storage API and sample implementation template from Anypoint Exchange.
Install this API on a Mule runtime that can access the storage solution.**[MISSING INFO - Confirm final name by GA. Add link if it's live before GA.]**
