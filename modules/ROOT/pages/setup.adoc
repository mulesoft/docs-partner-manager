...
The wrapper.conf file is located at `MULE_HOME/conf/wrapper.conf`. `MULE_HOME` is the environment variable which you set on server. It is the home directory of Mule runtime on the machine, usually on a Linux machine something like:

`/opt/{Mule installation}`.
...
. Include the created servers into the creaetd server group: *B2BServerGroup*. https://docs.mulesoft.com/runtime-manager/server-group-add[Server group]
