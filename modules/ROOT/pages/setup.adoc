...
Install this API on a Mule runtime that can access the storage solution.

. Make a note of the API address configurations, for example:

* host, port, path connection

* response timeouts

* authentication types: None, API Key, Basic, Client Credentials, OAuth Token
...
. Once the Payload Storage API is set up on the Mule runtime, open Anypoint Partner Manager, locate to your organizations profile (Host) page from the left navigation.
