...
[NOTE]
Use this configuration only when connecting to SFTP/FTP hosts that don't allow explicit file deletion after the first read. Using this configuration with other types of SFTP/FTP hosts can result in duplicate file processing.
|

| Skip empty files | Skip empty files during processing. | No
...
| Archival Path
a| Directory path on the SFTP server at which to store a copy of the file after processing.
