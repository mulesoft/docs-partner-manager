= Deploying Message Flows On-Premises
...
Each Partner Manager runtime application is deployed as an HTTP listener that listens on ports starting from port 10090. Leave ports 10090 through to 10100 open from a network standpoint before you deploy message flows.

NOTE: AS2 and HTTP receive endpoints deployed to on-premise Mule instances are deployed over plain HTTP. Using a load balancer exposes these endpoints to consumers as HTTPS endpoints.
