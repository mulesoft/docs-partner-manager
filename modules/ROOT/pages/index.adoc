...
* AS2, EDIFACT, and X12 Connectors, if your organization uses these standards and protocols.
* One or more of the user permissions for Partner Manager, assigned by your organization administrator.
...
* The required Mule runtime capacity, either vCore or messages/flows, to manage your B2B transaction workload needs
...
For other formats, such as XML, JSON, and CSV, - Partner Manager extracts the identifiers from data payloads or a message contact by dynamically executing the reference mapping supplied with the message type definition.

[[custom-attributes]]
=== Custom Message Attributes

User-defined attributes that are associated with Receive from Partners or Source at Host message types through reference mapping. As business transactions are processed, key data fields like Order Number, Invoice Number, and Shipment ID are extracted from messages. After a transaction is processed, users can find the message attribute label and value by searching transactions on the Activity page in Partner Manager or via API calls.
