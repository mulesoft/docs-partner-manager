= Not Ready for Review: Configure  Receive Settings for Inbound EDIFACT messages
...
Optionally configure acknowledgment settings for each Inbound EDIFACT message flow. To configure acknowledgement settings:

*Vijay, please make sure these descriptions are correct. Is there a better way to distinguish them from each other in the decriptions?*

* Enable *Send CONTRL technical acknowledgement for each transmission from <partner-name>*.
+
This acknowledgement indicates whether Partner Manager received and accepted the message.
+
* Enable the *Send CONTRL functional acknowledgement when messages fail to route to a message flow* setting.
+
This acknowledgement indicates whether Partner Manager received and accepted the message when there is a routing failure.

If you enable one of these options, you must either select the partner endpoint to which to send the acknowledgements or create a new endpoint for this purpose.
