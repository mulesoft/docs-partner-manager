...
. Select the partner for whom to configure the settings.
...
Optionally configure functional acknowledgment settings for each message flow. These settings apply only if the routing of a message flow fails.
...
. Select an endpoint from which the functional acknowledgment message is sent to the partner.
