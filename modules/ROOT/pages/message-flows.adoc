...
* Inbound
+
An inbound message flow receives EDI payloads from your partners, transforms them into your internal application message format, and sends the transformed messages to your backend systems.
+
For more information, see xref:inbound-message-flows.adoc[Inbound Message Flows].
+
* Outbound
+
An outbound message flow receives messages from your backend applications, transforms the messages to EDI, and sends the EDI messages to your partner.
+
