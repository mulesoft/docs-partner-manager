...
Use AI to generate a summary of the message payload. Make sure you've enabled Einstein in xref:cloudhub-deployment-settings.adoc[CloudHub 1.0], xref:cloudhub2-deployment-settings.adoc[CloudHub 2.0], or xref:rtf-deployment-settings.adoc[Runtime Fabric] deployment settings.

To generate a summary, open the source or target payload on the message details page and click *Summarize*. If this is the first time you're viewing a summary, accept the conditions to allow the use of Einstein.

The summary shows key details, such as the message type, sender, and receiver, as well as the contents of the message like the order number, customer name, and order details.
