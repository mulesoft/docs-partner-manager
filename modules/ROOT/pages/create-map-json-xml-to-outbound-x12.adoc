= Creating a Translation Map to Map an Application Format to Outbound X12

Create a template project in Anypoint Studio to import a template that creates a DataWeave map to transform application formats such as CSV, JSON, and XMLto outbound X12 transactions, then test the map locally.
...
To set the source CSV, JSON, or XML message type for the map:
...
. Click *Create type*.
. In the *Select metadata type* dialog window, from the *Type* drop-down menu, select *CSV*, *JSON*, or *XML*.
...
+
The application message structure (CSV, JSON, or XML) displays in the input pane of the Transform Message graphical view, and the X12 transaction set structure displays in the output pane of this view.
