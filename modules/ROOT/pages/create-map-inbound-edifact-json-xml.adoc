...
+
.Click Define Metadata start the mapping process.
...
+
.The EDIFACT and application message structures.
image::pm-inbound-map-2.png[Message structures]
+
.Click the Edit current target icon:
+
.Click the Edit current icon icon to access the file save funcionality.
