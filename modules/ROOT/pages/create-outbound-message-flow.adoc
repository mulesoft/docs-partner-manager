...
The receiving endpoint (Source at Host endpoint) in an outbound message flow receives application messages from the backend applications. Partner Manager supports the FTP, HTTP, HTTPS, and SFTP protocols for the receiving endpoint.

To configure the receiving endpoint:

. In the message flow, expand the *Source at <host-name>* section and click *Select*.
. In the *Select an endpoint: Source at <host-name>* window, complete either of these steps:
...
. In the *Source message type* window, complete either of these steps:
...
To configure a host reference ID, either select *Ignore* or select your organization's internal reference ID (such as the business unit name) as it appears in the data element of the source CSV, JSON, or XML message from your backend applications.

== Select the Translation Map

A translation map in an outbound message flow transforms outbound messages from your organization's enterprise application format to the format your partners expect. Maps are created using DataWeave and are imported to the xref:maps.adoc[Maps] page. For more information about creating maps, see xref:partner-manager-maps.adoc[Translation Maps].

To select the map:

. In the message flow, expand the *Map* section.
. Click *Select* and select the map from the table.

The map is added to the message flow. Only the mapping configurations that match your source and target message types are shown. Choosing a map configuration with a different source and target message type updates the corresponding message type in your message flow.

In the map for an outbound message flow, you can access source message attributes, which makes them available in the message payload. The attributes that you can access depend on the protocol of the source endpoint. For more information, see xref:access-source-message-attributes.adoc[Accessing Source Message Attributes]. 
...
For the X12 and EDIFACT message types, configuring the target message type also involves these additional steps:

* Optionally configuring acknowledgments
* Optionally creating custom X12 or EDIFACT settings
* Selecting X12 or EDIFACT identifiers to use for routing the outbound messages

To configure the target message type:

. Expand the *Message type* section and click *Select*.
. In the *Select message type* window, complete either of these steps:
...
. In the *<format> Identifiers* section, select the sender and receiver identifiers to use on either the X12 ISA and GS segments or the EDIFACT UNB segments.
+
...
=== Optionally Apply Custom Validation Settings to X12 and EDIFACT Formats

By default, Partner Manager uses the X12 or EDIFACT send settings configured for the partner or third-party connection for message validation, delimiters, and character encoding.

To apply custom validation settings and delimiters to the outbound message flow, follow these steps:

. In the *X12 or EDIFACT Settings* section, select either *Custom X12 settings* or *Custom EDIFACT settings* and provide custom configuration values.
. In the *<format> Identifiers* section, select the sender and receiver identifiers to use on the X12 ISA and GS segments or the EDIFACT UNB segments.
+
These identifiers are configured in the host and partner profiles.

==== Configure Custom Settings for the ISA11 Element

Configure the ISA11 element values for outbound X12 messages in X12 custom settings.

* For ISA 4010 or earlier, set the ISA11 interchange control standards identifier on the *General* tab.
+
image::pm-custom-x12-settings-general.png["The ISA11 interchange control standards identifier in the custom X12 settings"]

* For ISA 4020 or later, set the ISA11 repetition separator on the *Character set & separators* tab.
+
image::pm-custom-x12-settings-char-set.png["The ISA11 repetition separator in the custom X12 settings"]
...
Partner Manager supports the AS2, FTP, SFTP, HTTP, and HTTPS protocols for Send to Partners endpoints.
