...
When you create a new Receive from Partners or Source at Host message type, you can associate it with a DataWeave reference map. This is mandatory for Receive from Partners message types formatted with JSON or XML and for all Source at Host message types. Reference maps can include custom message attributes that get extracted from the message payloads at runtime. The extracted attributes are visible in the payload and are available for searching and tracking.
...
** Receive from Partners message types formatted with JSON or XML
* Optional for:
** Receive from Partners message types formatted with EDIFACT or X12 EDI
** All Source at Host message types
