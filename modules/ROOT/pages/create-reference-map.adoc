...
When you create a new Source at Host or Receive from Partners message type formatted with JSON or XML, you must associate the message type with a DataWeave reference map. This is optional for Receive from Partners message types formatted EDFIACT or X12. Reference maps can include custom message attributes that get extracted from the message payloads at runtime. The extracted attributes are visible in the payload and are available for searching and tracking.
