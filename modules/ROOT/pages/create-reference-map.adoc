...
If the output of a reference map does not conform to the validation schema, the custom message attributes won't appear in the Partner Manager GUI. To ensure that the output of a reference map conforms to the validation schema, you can add a *Validate schema* operation in an Anypoint Connector for X12 EDI (X12 Connector) app, as shown in these Exchange examples:
