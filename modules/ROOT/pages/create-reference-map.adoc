...
The output of a reference map contains the following fields and must validate against the apm-reference-attributes-schema.json or pm-reference-attributes-schema.xml schema, which come with Anypont Partner Manager:
...
== Schemas that Validate the Reference Map Output

Depending on the output format, the reference map output must comply with one of the following schemas:
...
* The apm-reference-attributes-schema.xml
