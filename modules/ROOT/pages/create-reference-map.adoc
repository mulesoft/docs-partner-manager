...
[NOTE]
While this schema allows `hostReferenceId` to be nullable to accommodate all message types, the field remains mandatory for Receive from Partners flows using JSON, CSV, or XML formats. Refer to the field requirements table above for message-type-specific requirements.
| businessDocumentKey.
