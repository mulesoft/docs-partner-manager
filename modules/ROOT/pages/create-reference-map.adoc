...
[NOTE]
The JSON schema allows `hostReferenceId` to be null because it's shared across message types. For Receive from Partners message types that use JSON, CSV, or XML, `hostReferenceId` is still required.
| businessDocumentKey.
