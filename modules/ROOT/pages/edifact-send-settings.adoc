...
|If selected, enforces EDIFACT code set validations, as defined in the EDIFACT schemas.
|Disabled
|===
...
|*
|Segment terminator
|Character used to terminate segments.
|'
|Release character
|Release character
|?
...
a|
Character used for line endings between segments to improve the readability of the output message text. Possible values include:
...
Line Feed
|Test Indicator
a| Indicates whether this is a test or production message. *???*When you select *Test* from the drop-down, sets UNB-11 to `1`, which indicates that this is a test message.
|Production *Still true???*
