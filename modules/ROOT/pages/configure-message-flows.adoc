...
. To Select an Existing Partner from the *Select a partner* dialog, search for the trading partner by name. Once you find a match, choose the partner name and click *Select*.
...
TIP: Clicking each section of the message flow expands it. A green checkmark next to any a section indicates that it is complete.

[start=3]
. Configure the source endpoint where you receive the transmission by clicking the *Receiving endpoint* section to expand it and click *Select*. Search and select an existing endpoint and click *Select*, or create a new endpoint.
...
[start=5]
. Select the source message type by clicking the *Source message* section to expand it. Click *Select* and either search and select an existing message type, or create a new message type.
...
[start=7]
. Configure acknowledgments (optional, only if the partner requires them for X12 messages), the type of acknowledgment, and the endpoint to which you send it. Refer to the following information:
...
* xref:document-types.adoc[Message (Document) Types]
[start=11]
. Set up a target endpoint by clicking *Select*. Either search and select and existing endpoint, or create a new endpoint.
