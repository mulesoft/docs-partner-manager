...
+
. (Optional) Click the *View* link to edit the validation settings and then click the hostname.
...
+
. (Optional for X12 partner requirements) Configure the type of functional acknowledgment, and the endpoint to which you send it. Refer to the following for endpoint information:
+
