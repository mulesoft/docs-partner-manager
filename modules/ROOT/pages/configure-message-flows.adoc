...
* Alternatively from the *Select a partner* dialog, you can create a new Trading Partner by clicking *New*. This is the same as xref:configure-partner.adoc#create-a-trading-partner[Create a Trading Partner] except that when you are finished, you'll click *Create Partner and Message Flow*. You can add more information to the parter now or later.
...
+
. (Optional) Click the *View* link to edit the validation settings and then click the hostname. For more information, see [xref] to X12 settings.
