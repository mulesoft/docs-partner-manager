...
You can create and configure a message flow at the time you create a trading partner or afterwards. Once you deploy the message flow you configured, it's best to test it.
...
NOTE: The deployment process can take several minutes. Do not log out of the Anypoint platform while the deployment is in progress as it causes the deployment to terminate and fail. You can, however, continue to work within the Anypoint platform during this time.
...
There are a couple of reasons why you should send test messages through the message flow after you deploy:

* Ensure that you are receiving messages and any acknowledgments from your partners.

* Make sure that the DataWeave map is working and your backend systems receive the transformed message.

You can test the flow by deploying it in a sandbox environment, then undeploying it, and finally deploying it to a production environment.

Test your message flow in your environment:
...
. Scan the Transmission Flow sections for an exclamation point (!) in any of the rectangle boxes next to the displayed flow stages as this indicates there is an issue with the transmission.

. If view payloads are enabled, review the received payload by clicking *View payload* next to the Received transmission section.

. If you had any TA1 and functional acknowledgments (999 or 997) set, review and confirm that they are there.

. Check that the transmission routed messages to the correct message flows.

. Ensure that the messages are routed to the target system. Specifically check:

** That it was successfully delivered to the target endpoint.

** That the message was transformed correctly.
