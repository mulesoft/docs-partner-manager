...
EDIFACT message type, such as `ORDERS`, `DESADV`, or `INVOIC`.
 |  | Yes
|Reference identifiers and attributes mapping | Uploaded DataWeave map that extracts the reference identifiers and message attributes from the payload received from the partner:
{sp} +
{sp} +
The extracted values for the reference identifiers are used for dynamic routing into the appropriate message flow.
| |No
...
|Reference identifiers and attributes mapping| DataWeave map that extracts the reference identifiers, business key, and message attributes from the payload received from the partner.
{sp} +
{sp} +
...
|Message type |X12 transaction set number, such as `850` (purchase order) or `214` (shipment status). | | Yes
|Reference identifiers and attributes mapping | Uploaded DataWeave script with a mapping that extracts the reference identifiers, business key, and message attributes from the payload received from the partner.
{sp} +
{sp} +
...
* xref:use-custom-attributes.adoc[Using Custom Message Attributes]
