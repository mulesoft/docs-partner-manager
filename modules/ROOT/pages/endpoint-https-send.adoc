...
|Maximum time, in milliseconds, for which the request element blocks the execution of the flow that is waiting for the HTTP response. The defauilt is 15000 milleseconds seconds.
|x
...
|Maximum number of reconnection attempts to make. If you don’t want Partner Manager to retry after an initial connection failure, set this value to `0`. The default is 3.
|x
