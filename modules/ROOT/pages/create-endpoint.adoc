...
| Source at host | FTP, HTTP(S), SFTP | Host
| Target at host | FTP, HTTP(S), SFTP | Host
| Receive from partners | AS2 |Host
|  | FTP, HTTP(S), SFTP | Host, partner, 3rd party connection
...
Each unique AS2 and HTTP(S) receive endpoint configurations result in the deployment of their own runtime applications. You can view the Runtime application name corresponding to a given configuration in Partner Manager, from the endpoint detail page.
