...
You can create new host-owned, partner-owned, and third-party-connection-owned endpoints from the *Host* or *Partner* page or by using the Message Flow wizard.

The following table describes the endpoint types, along with their supported protocols and owners:



== Create Endpoints Using the Partners Page
...
== Use the Host Page to Create Endpoints

Use the *Host* page to create new end and receive endpoints owned by the host:
...
When creating endpoint types using the Message Flow wizard, the ownership of the endpoint is automatically assigned to the host, partner, or third party connection from which the endpoint was created. Using the Message Flow wizard, you can xref:configure-message-flows.adoc[Create and Configure an Inbound Message Flow] or xref:create-outbound-message-flow.adoc[Create and Configure an Outbound Message Flow].
...
*Vijay, should Backend be labeled Host instead?*

|===
|Type | Protocol | Can be owned by
| Receive from partner | AS2 | Host
| Receive from partner | FTP, SFTP | Host, partner, 3rd party connection
| Send to partner | AS2 | Partner, 3rd party connection
|===
