...
The endpoints for AS2 Send, FTP Receive, FTP Send, HTTP(s) Receive, HTTP(s) Send, SFTP Receive, and SFTP Send are always managed within a single generic endpoint service for the protocol & direction.
...
* xref:endpoint-ftp-receive-target.adoc[FTP Receive Endpoint Settings]
