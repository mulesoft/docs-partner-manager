...
. In the *Acknowledgement* section, select the checkbox next to *Send 997 functional acknowledgment (X12)* or *Send CONTRL functional acknowledgement (EDIFACT)*.
. Click *Select*.
...
After you import the translation map for an inbound message flow, you can add additional source message attributes to the map. This makes these attributes available in the message playoad. The attributes that you can add depend on the protocol of the source endpoint. For more information, see xref:add-additional-source-message-attributes.adoc[Adding Additional Source Message Attributes to Translation Maps]. 
