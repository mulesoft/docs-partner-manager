...
You can create and configure an inbound message flow to receive EDI messages from your trading partners, including trading partners that are 3rd party aggregators. When you create an inbound message flow, you can validate and transform the inbound messages and then send the translated message (JSON or XML) to your backend applications.
...
. Either select an existing partner from the list or select *New* to xref:configure-partner.adoc#create-partner[Create a partner].
+
...
* Click *New* to create a new endpoint, as described in xxx.
+
