...
If the source message type is:

* X12, select the partner and host organization's ISA (Interchange) identifiers from the dropdown menu.

* EDIFACT, select the partner and host organization's UNA (Interchange) identifiers from the dropdown menu.

* CSV, JSON, or XML, select the partner and host organization's reference identifiers from the dropdown menu.

To route inbound messages from partners matching any of the partner or host identifiers, select *All identifiers* from the dropdown menu. For more information about the order of precedence in inbound message routing, see xref:inbound-message-routing.adoc[Inbound Message Routing].


== Select the Translation Map

A translation map in an inbound message flow transforms your inbound message format to your organization's enterprise application format. Maps are created by using DataWeave and are imported to the xref:maps.adoc[Maps] page. For more information about creating maps, see xref:partner-manager-maps.adoc[Translation Maps].
...
In the map for an inbound message flow, you can access source message attributes, which makes them available in the message payload. The attributes that you can access depend on the protocol of the source endpoint. For more information, see xref:access-source-message-attributes.adoc[Accessing Source Message Attributes]. 
