...
If you want to process cXML messages with <DOCTYPE> tags, you must set the `minMuleVersion` to 4.5.0 or higher and upgrade your runtime environment. For more information, see Partner Manager deployment settings for xref:cloudhub-deployment-settings.adoc[CloudHub], xref:cloudhub2-deployment-settings.adoc[CloudHub 2.0], and xref:rtf-deployment-settings.adoc[Runtime Fabric].
