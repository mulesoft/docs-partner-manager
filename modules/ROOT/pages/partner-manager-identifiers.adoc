...
. If you selected `EDIFACT-UNB` or `X12-ISA` as the identifier type, you also need to select a qualifier.
...
|Yes
| Identifier value
| EDIFACT-UNB ID for your partner, which can be up to 15 characters. Your partner supplies the EDIFACT Interchange Sender and Receiver IDs during onboarding.
| Yes
...
Complete this field to configure a reference ID (DUNS number) identifier:
...
|Identifier Value
|DUNS number for your partner, which is a nine-digit number that corresponds to the sender or receiver ID. Your partner supplies this number during onboarding.
...
|Identifier value
|EDI X12-GS ID for your partner, which can be up to 15 characters. Your partner supplies the EDI X12 Group Sender and Receiver IDs during onboarding.
|Yes
|===
...
|Identifier value
|EDI X12-ISA ID for your partner, which can be up to 15 characters. Your partner supplies the EDI X12 Interchange Sender and Receiver IDs.
...
You can delete identifiers that are no longer used in a message flow. Review these considerations prior to deleting an identifier:

* You can't delete the last host identifier of its type.
* Deleting an existing host or partner identifier affects the identification and routing of B2B messages to the associated partner.
* If you replace an identifier with another identifier, record the existing identifier's value and qualifier type.
...
* An outbound purchase order XML message from your ERP system to your partner, Mythical Supplier, has this content in the payload:
+
`<VendorName>MYTHICAL SUPPLIER, LLC</VendorName>`
+
* An outbound payment remittance advice JSON message from your financial system to the same partner has this content in the payload:
+
`"PayeeName": "MYTHICAL ENTERPRISES"`

For Partner Manager to route these outbound transactions to the appropriate message flows for Mythical Supplier, add these values as partner reference IDs to the partner's profile page. Create a new identifier and select the type as *Reference ID*.
