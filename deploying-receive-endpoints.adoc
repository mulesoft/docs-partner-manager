...
=== External URL to share for an HTTP or HTTPS Receive Endpoint

The external URL that you can share with your partners for an HTTP or HTTPS endpoint is `+http://{DLB-domain}/{Input-path}/{base-path}/{message-type}+` or `+https://{DLB-domain}/{Input-path}/{base-path}/{message-type}+`.
